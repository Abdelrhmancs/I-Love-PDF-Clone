...
namespace I_love_pdf.Controllers
{
    ...
    public class Watermark : ControllerBase
    {
        ...
        public async Task<IActionResult> WatermarkPdf(List<IFormFile> files, [FromForm] string text)
        {
            ...
            {
                return BadRequest("write a watermark text");
            }
            ...
            var outputFile = Path.Combine(uploadFolder, "addWatermark_" + file.FileName);
            ...
            //script
            var filepath = @"D:\Backend\I-Love-PDF-Clone\Pdf-Scripts\Watermark.py";
            var psi = new ProcessStartInfo
            {
                FileName = "Python",
                Arguments = $"\"{filepath}\" \"{inputFile}\" \"{outputFile}\" \"{text}\"",
