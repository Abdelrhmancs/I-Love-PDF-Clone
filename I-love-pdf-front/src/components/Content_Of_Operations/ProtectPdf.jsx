...
const ProtectPdf = () => {
  ...
  const handelUploadFile = async (password) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5147/api/ProtectPdf/protect-pdf",
        formData,
        {
      ...
      a.href = Url;
      a.setAttribute("download", "addPassword_" + file.name);
  ...
  if (success) {
    ...
    if (!okGetFile) {
      ...
      return (
        <>
          <div className="flex flex-col justify-center items-center p-3  ">
            <h1 className="font-bold text-[42px] text-[#33333b]">Protect PDF</h1>
            <p className="text-[22px] text-gray-700">
            Protect PDF files with a password. Encrypt PDF documents to prevent unauthorized access.
            </p>
